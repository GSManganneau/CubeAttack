...
public Decor(){
	...
	//____________________________________Création aléatoire des hauteurs de chaque rectangle servant d'obstacle__________________
	...
	public void afficherDecor(double[][] tab, double[][] tab1, Graphics g) {
		...
		for (int j = 0; j <= 99; j++) {
			g.fillRect((int) tab[1][j], 0, 20, (int) tab[0][j]);
			g.fillRect((int) tab1[1][j], (int) tab1[0][j], 20,
					500 - (int) tab1[0][j]);
			if(vitesse == 100){
			g.setColor(bleue);
			}
			...
			g.drawRect((int) tab[1][j], 0, 20, (int) tab[0][j]);
			g.drawRect((int) tab1[1][j], (int) tab1[0][j], 20,
					500 - (int) tab1[0][j]);
			g.setColor(Color.GRAY);
		}
	}
